...
module.getSkin = function()
	...
	local headBlendData = GetPedHeadBlendData(playerPed);
	...
	local _, body_blemishes, _, _, _, body_blemishes_opacity = GetPedHeadOverlay(playerPed, 11);
	if body_blemishes == 255 then
		body_blemishes = -1;
	end

	local _, body_blemishes_effect, _, _, _, body_blemishes_effect_opacity = GetPedHeadOverlay(playerPed, 12);
	...
	local skin = {
		-- head blend data
		mom = headBlendData.shapeFirst,
		dad = headBlendData.shapeSecond,
		face_weight = headBlendData.shapeMix,
		skin_weight = headBlendData.skinMix,

		-- ped face features
		nose_width = GetPedFaceFeature(playerPed, 0),
		nose_peak = GetPedFaceFeature(playerPed, 1),
		nose_length = GetPedFaceFeature(playerPed, 2),
		nose_bone_curveness = GetPedFaceFeature(playerPed, 3),
		nose_tip = GetPedFaceFeature(playerPed, 4),
		nose_bone_twist = GetPedFaceFeature(playerPed, 5),
		...
		cheeks_depth = GetPedFaceFeature(playerPed, 9),
		cheeks_width = GetPedFaceFeature(playerPed, 10),
		...
		chin_shape = GetPedFaceFeature(playerPed, 17),
...
local applySkin = function(skin)
	...
	SetPedFaceFeature(playerPed, 3, skin.nose_bone_curveness)
	SetPedFaceFeature(playerPed, 4, skin.nose_tip)
	SetPedFaceFeature(playerPed, 5, skin.nose_bone_twist)
	...
	SetPedFaceFeature(playerPed, 9, skin.cheeks_depth)
	SetPedFaceFeature(playerPed, 10, skin.cheeks_width)
	...
	SetPedFaceFeature(playerPed, 17, skin.chin_shape)
	SetPedFaceFeature(playerPed, 18, skin.chin_hole)
	SetPedFaceFeature(playerPed, 19, skin.neck_thickness)

	SetPedHeadOverlay(playerPed, 0, skin.blemishes, skin.blemishes_color) -- Blemishes + opacity
	SetPedHeadOverlay(playerPed, 1, skin.beard, skin.beard_color) -- Beard + opacity
	SetPedHeadOverlay(playerPed, 2, skin.eyebrows, skin.eyebrows_color) -- Eyebrows + opacity
	SetPedHeadOverlay(playerPed, 3, skin.age, skin.age_color) -- Age + opacity
	SetPedHeadOverlay(playerPed, 4, skin.makeup, skin.makeup_color) -- Makeup + opacity
	SetPedHeadOverlay(playerPed, 5, skin.blush, skin.blush_color) -- Blush + opacity
	SetPedHeadOverlay(playerPed, 6, skin.complexion, skin.complexion_color) -- Complexion + opacity
	SetPedHeadOverlay(playerPed, 7, skin.sun, skin.sun_color) -- Sun Damage + opacity
	SetPedHeadOverlay(playerPed, 8, skin.lipstick, skin.lipstick_color) -- Lipstick + opacity
	SetPedHeadOverlay(playerPed, 9, skin.moles, skin.moles_color) -- Moles/Freckles + opacity
	SetPedHeadOverlay(playerPed, 10, skin.chest_hair, skin.chest_hair_color) -- Chest Hair + opacity
	...
	SetPedHeadOverlayColor(playerPed, 4, 2, skin.makeup_color, skin.makeup_color2) -- Makeup Color
	SetPedHeadOverlayColor(playerPed, 5, 2, skin.blush_color) -- Blush Color
	SetPedHeadOverlayColor(playerPed, 8, 1, skin.lipstick_color, skin.lipstick_color2) -- Lipstick Color
	SetPedHeadOverlayColor(playerPed, 10, 1, skin.chest_hair_color) -- Torso Color
